...
| 18 | ✅ **Erledigt:** `.rules/eslint-overrides-for-monads.md` angelegt; zentrale `MONAD OVERRIDES`-Section in `eslint.config.js` ergänzt (`option/**`, `callback/**`, `exceptions/core/exception.ts`). Alle pro-Datei- und Inline-Disables in `src/` entfernt — Quelldateien sind jetzt frei von repetitiven Disable-Headern. | F-25, F-05, F-51 | `eslint.config.js`, `.rules/eslint-overrides-for-monads.md` (neu), Cleanups in `option/**`, `callback/**`, `exception.ts`, `optionBase.ts` |
...
| 20 | ✅ **Erledigt:** `.rules/intra-feature-imports.md` angelegt. Konvention: intra-Feature-Importe sind relativ zur Quelldatei; Cross-Feature-Importe ausschließlich über den `essentials:<feature>`-Alias. Bestehender Code befolgt die Regel bereits — keine Code-Änderungen nötig. | F-01, F-24, F-38 | `.rules/intra-feature-imports.md` (neu) |
...
|---|---|---|---|
| 25 | ✅ **Erledigt:** Cross-Monaden-Brücke ergänzt — `Option.fromResult(res)` (Ok→Some, Err→None, Error wird verworfen) und `Result.fromOption(opt, error)` (Some→Ok, None→Err mit injiziertem Error). <br><br>**Form-Entscheidung:** Statische Methoden auf den Namespaces (`Option`/`Result`) statt Instanz-Methoden (`opt.toResult(err)` / `res.toOption()`). Begründung: Instanz-Methoden hätten einen Runtime-Cross-Feature-Loop zwischen `OptionBase` und `ResultBase` reproduziert — genau die Klasse von Loop, die #28 mit dem Lazy-Setter-Pattern aufgelöst hat. Der Namespace-Ansatz benutzt ausschließlich `import type`-Cross-Feature-Imports, die der TS-Compiler beim Emit eliminiert. Damit bleiben beide Module loop-frei. Ergonomisch eine Spur weniger flüssig (`Option.fromResult(res)` statt `res.toOption()`), aber strukturell deutlich robuster. <br><br>**Semantik:** `fromOption` verlangt einen expliziten Error-Parameter, weil `IOption` keine Info über das *Warum* einer Absenz trägt. `fromResult` ist bewusst lossy. Beide Doc-Comments machen das explizit. <br><br>**Tests:** 4 Cases pro Richtung — Some/None bzw. Ok/Err Branches, verbatim-Value-Preservation, Lazy-Evaluation der unbenutzten Error-Path-Semantik. Build verifiziert. Test-Bilanz: 293 → 301. | F-36 | `option.ts`, `option.test.ts`, `result.ts`, `result.test.ts` |
| 26 | ✅ **Erledigt (teilweise — F-47 voll, F-46 verschoben):** 5 neue HTTP-Subklassen ergänzt: `MethodNotAllowedException` (405), `UnprocessableEntityException` (422), `TooManyRequestsException` (429), `BadGatewayException` (502), `GatewayTimeoutException` (504). Alle erben von `HttpStatusException` und tragen ihren `static httpStatus`. <br><br>**Factory-Refactor:** `HttpStatusExceptionFactory` von Switch-Cascade auf `Map<number, Constructor>`-Lookup umgestellt. Die Map wird aus einem einzigen `httpStatusExceptions`-Registry-Array gebaut, das wiederum die `static httpStatus`-Felder auswertet. Eine neue HTTP-Subklasse einzufügen ist jetzt eine Zeile im Registry — kein Switch-Update mehr nötig. Unknown-Status-Fallback (`HTTP <status> Error`-Message) bleibt unverändert; das gepinnte 418-Teapot-Verhalten ist intakt. <br><br>**F-46** (statische Klasse als Anti-Pattern → freistehende Funktion) bewusst nicht adressiert: Public-API-Änderung. Bleibt als separate Aufgabe offen, wenn Bedarf da ist. <br><br>**Test-Bilanz:** 235 → 293 (+58). 5 neue Subklassen × 8 Spec-Cases = 40 in `subclasses.test.ts`; Factory-Tests von 7 auf 12 Status-Codes erweitert. Bestehende pinned Tests unangetastet. <br><br>Aufräumarbeit am Rande: 2 ungenutzte `eslint-disable @typescript-eslint/no-magic-numbers` aus `exception.test.ts` und `httpStatusExceptionFactory.test.ts` entfernt. | F-46 (offen), F-47 (erledigt) | 5 neue Subklassen-Dateien, `httpStatusExceptionFactory.ts`, `httpStatusExceptionFactory.test.ts`, `subclasses.test.ts`, `exceptions/index.ts`, 2 Test-Files Hygiene |
| 27 | ✅ **Erledigt:** Neue abstrakte Zwischenklasse `HttpStatusException` zwischen `Exception` und den 7 HTTP-Subklassen eingezogen. Jede HTTP-Subklasse trägt jetzt `static readonly httpStatus`: `BadRequest=400`, `Unauthorized=401`, `Forbidden=403`, `NotFound=404`, `Conflict=409`, `InternalServerError=500`, `ServiceUnavailable=503`. `InvalidStateException` bleibt bewusst direktes Kind von `Exception` — sie ist library-intern (z.B. `Option.unwrap` auf None) und hat keine HTTP-Semantik. <br><br>**Konsumenten-Pattern**: Statt `instanceof`-Cascade reicht jetzt `if (err instanceof HttpStatusException) { ctx.status((err.constructor as typeof HttpStatusException).httpStatus); }`. <br><br>Vererbungskette `Exception → HttpStatusException → ConcreteHttpException` lässt alle bestehenden `instanceof Exception`-Checks weiter passieren — kein Breaking Change. <br><br>Tests: `subclasses.test.ts` um eine `IOption<number>`-Spec-Entry pro Subklasse erweitert (`Some(NNN)` für HTTP, `None()` für `InvalidState`). Pro Subklasse zwei zusätzliche Cases: HTTP-Klassen prüfen `instanceof HttpStatusException` und `httpStatus`-Wert; `InvalidState` prüft explizit `not.toBeInstanceOf(HttpStatusException)`. Gesamt jetzt 243/243 Tests grün. | F-48 | `httpStatusException.ts` (neu), 7 HTTP-Subklassen, `subclasses.test.ts`, `exceptions/index.ts` |
| 28 | ✅ **Erledigt:** `isOption` / `isResult` strikter via `value instanceof OptionBase` / `instanceof ResultBase`. Duck-typed Eingaben (Plain-Objekte mit `isSome`/`isNone`-Booleans, fremde Monad-Likes, JSON-Roundtrips) liefern jetzt korrekt `false` — der Guard verifiziert die volle Prototype-Kette statt nur die Shape. Test-Suiten in `isOption.test.ts` (8 Tests) und `isResult.test.ts` (8 Tests) flippten die duck-typed Cases auf `false` und prüfen damit die strikte Semantik. <br><br>**Voraussetzung:** Der Switch zog einen latenten zirkulären Modul-Init-Loop ans Licht (`option.ts` ↔ `optionBase.ts` über `Some`/`None`; analog `result.ts` ↔ `resultBase.ts`). Vorher funktionierte die Reihenfolge nur, weil `isOption` `OptionBase` ausschließlich type-position importierte → vom TS-Compiler eliminiert. Mit dem `instanceof`-Wert-Import wurde der Loop scharf und crashte mit `TypeError: Class extends value undefined`. <br><br>**Loop-Auflösung** (separater Commit `0ddc280`): Lazy-Setter-Pattern. `optionBase.ts` exportiert ein internes `optionFactories`-Objekt mit Slots (`some`, `none`), die defensiv mit Throw-Defaults belegt sind. `option.ts` befüllt die Slots direkt nach den Factory-Definitionen. Methoden auf `OptionBase` (`map`, `andThen`, `filter`, `and`) lesen die Slots zur **Aufruf-Zeit**, nicht zur Init-Zeit — damit hat `optionBase.ts` keinen Top-Level-Import auf `option.ts` mehr. Analog `resultFactories` in `resultBase.ts`. Beide Barrel-Dateien zusätzlich in robuster Reihenfolge: `models` → `core` → `guards`. | F-17, F-31 | `isOption.ts`, `isOption.test.ts`, `isResult.ts`, `isResult.test.ts`, `optionBase.ts`, `option.ts`, `resultBase.ts`, `result.ts`, `option/index.ts`, `result/index.ts` |
| 29 | ✅ **Erledigt:** `OptionBase.getValue()` und die Overrides in `SomeOption`/`NoneOption` von `public` auf `protected` markiert. Einziger externer Aufrufer war `option.ts:toJsonObject` — auf `IOption.match` umgestellt (Public-API, semantisch sauberer). Historische rekursive Envelope-Shape von `toJsonObject` auf einer `None` 1:1 erhalten (`toJsonObject(undefined)` im `onNone`-Branch reproduziert das alte Verhalten von `toJsonObject(value.getValue())` exakt). F-14 bleibt damit pinned bis #30. <br><br>**Beobachtung am Rande (außerhalb #29-Scope):** `SomeOption.value` ist `public readonly` und damit ein analoges Public-API-Leck. Sollte in einem Folge-Refactor genauso weggekapselt werden. | F-19 | `optionBase.ts`, `someOption.ts`, `noneOption.ts`, `option.ts` |
| 30 | ✅ **Erledigt (BREAKING CHANGE — Voll-Variante):** Alte API `toJsonObject` / `toJsonString` / `toOption` (samt `Option.toJsonObject` / `Option.toJsonString` / `Option.toOption`-Aliasse) komplett entfernt. Auch der interne `isJsonString`-Helper ist weg. Ersatz: `Option.serialize<T>(opt: IOption<T>): SerializedOption<T>` und `Option.deserialize<T>(env: SerializedOption<T>): IOption<T>` — ein lossless Round-Trip-Paar mit klar abgegrenzter Verantwortung. <br><br>**Neuer Wire-Format-Type** `SerializedOption<T> = { isSome: true; value: T } \| { isSome: false }` als Discriminated Union in `option/models/SerializedOption.ts`. Bewusst **ohne** `value`-Schlüssel auf der None-Seite — damit überlebt der Envelope `JSON.stringify`/`JSON.parse` verlustfrei (kein `value: undefined`-Verlust unter JSON), und `Some<falsy>` (`Some(0)`, `Some('')`, `Some(false)`) round-trippt jetzt korrekt. F-14 ist damit gefixt, nicht mehr pinned. <br><br>**Bewusst nicht mehr im Scope:** rekursives Walken durch fremde Strukturen (Arrays, Plain-Objekte, JSON-Strings). Die alte `toOption` hat all das vermischt und ihre Signatur belog die Konsumenten (F-13). `serialize`/`deserialize` operieren ausschließlich auf einer einzelnen Option. Wer geschachtelte Strukturen serialisieren will, mappt selbst über sein Datenmodell und ruft pro Leaf — das hält den Vertrag ehrlich. F-13 und F-15 sind damit ebenfalls gefixt (durch Removal). <br><br>**Defensive Validation** in `deserialize`: Wenn ein Aufrufer per `as any` etwas reinwirft, das kein Envelope ist, wirft es `InvalidStateException` statt stillschweigend zu zerbrechen. <br><br>**Test-Bilanz:** 301 → 293. Alle pinned F-14/F-15-Tests im `option.test.ts` (drei describe-Blöcke `toJsonObject` / `toJsonString` / `toOption` + drei alte Façade-Aliasse) gelöscht. Stattdessen drei neue describe-Blöcke: `Option.serialize` (4 Cases), `Option.deserialize` (3 Cases), und ein dedizierter `Option.serialize / Option.deserialize round-trip`-Block (4 Cases) der die Lossless-Garantie inklusive `JSON.stringify`/`JSON.parse`-Schritt für Some&lt;number&gt;, None, Some&lt;falsy&gt; und None-via-JSON beweist. | F-13, F-14, F-15 (alle gefixt) | `option/models/SerializedOption.ts` (neu), `option/core/option.ts`, `option/index.ts`, `option/core/option.test.ts` |
